...
| M66 | 目标类型传播收口为完整矩阵：结构体字段默认值改走目标检查；声明、赋值、字段、return、函数/方法实参、array、result、match/三元与数值复合表达式递归传递目标。类型不符诊断直接复用已推断的实际类型，不再为拼消息二次执行无目标检查 | `try_from` 在不存在转换时必须等效于原表达式；二次检查会把合法回退篡改成「需要目标类型上下文」 | 单一矩阵正向用例覆盖全部目标槽；数组/result/match/构造字段/函数与方法实参、字段默认值逐项比较直接表达式与 `try_from` 的诊断等价性 |
| M67 | 原生间接调用统一接受任何携带完整函数签名的被调表达式；删除只允许标识符、`this` 和函数字面量的形态分派，三元与 match 产生的函数值可直接调用 | sema 已接受并保留这些表达式的共同函数签名，后端形态白名单却在合法程序上报 Phase 3 占位错误 | 三元函数值、match 函数值与立即调用函数字面量三条原生法律共享同一调用路径 |
| M68 | sema 的目标表达式检查改用类型化错误通道，类型不一致直接携带 expected、actual、Span；字面量越界保留独立类型分支。删除 `expected_actual`、结构体默认值的消息前缀拆解和绑定的消息前缀分类，领域调用方直接按错误变体生成原中文诊断 | 项目禁止用诊断字符串传递控制流；格式化后的中文消息不是稳定的类型信息源，二次无目标检查还会破坏 `try_from` 回退语义 | 既有 247 条测试数量不变；扩展 `try_from_fallback_preserves_outer_slot_diagnostics`，逐字锁定数组、result、match、复合运算、函数实参、结构体构造、array.push、扩展方法实参与字段默认值，保证调用方不重检表达式 |
| M69 | sema 输出 `CheckedProgram` typed HIR，每个表达式固化最终 `Ty`，调用表达式同时固化已解析目标；codegen 入口恰调用一次 `project_ty(&CheckedProgram)` 建立只读 `VTy` 投影表，发射阶段只查表并按 resolved target 工作。物理删除 `types_proj.rs`、`static_vty`、`infer_ret_vty` 与 `VTy::Other`，匿名函数字面量返回推断收归 sema | 后端按 AST 形态、名字和函数体重复猜类型会与目标传播及遮蔽解析漂移；静默 `Other` 会把 sema 缺口伪装成 I64 ABI | 全表达式形态 HIR 投影法律逐节点拒绝未确定类型并要求调用目标；完整 `Ty` 递归投影矩阵覆盖函数、result、array、iterator 与显式 Unknown 状态；既有原生法律保持 |

## resolved HIR / fail-closed 后端 / 当前语料收口（2026-08-28）

| # | 变更 | 依据 | 安全性 |
|---|---|---|---|
| M70 | 零参数命名函数与 `this` 的直接引用可省略括号；显式 `five()` 继续作为正式文法，函数值作为参数时用 `f(g)` 保持值语义 | no-paren 法律已经落地，但专题和当前规范仍错误宣称必须写 `five()` | `noparen_laws` 同时锁定省略与显式两种形式；函数 display 黄金改用显式参数括号 |
| M71 | stable ID 闭环补齐：`CheckedProgram` 保存 `main_id`；用户方法目标必含 `MethodId`；字段访问/赋值与结构体构造参数均固化字段索引；HIR capture 以 `BindingId` 传播 | backend 重新按 `main`、方法名、构造 label 或字段名查找会在 shadow/改名时漂移 | stable-ID、struct、method、closure capture 法律覆盖；backend 不再拥有对应名字解析分支 |
| M72 | sema 的可选 call target 与 implicit-zero 信息收回 lowering 临时态；最终 `Call` / `MethodCall` 直接持有不可空且按节点分类的 target。codegen god-file 按 emit/native-runtime 子模块拆分，并删除 emit wildcard re-export barrel | resolved HIR 不应保留“调用但无 target”或“普通调用携带方法”的无效状态；物理拆文件后仍 wildcard 平铺会继续制造偶然可见性 | HIR validate/visit/capture 单元测试与全目标编译锁定；跨 emitter 依赖改为显式 owner import |

...
- pointer equality / ordering / difference 的静态类型兼容性只由 `sema/exprs/operators.rs::binary_result_type` 决定；运行时 check requirement 由 `hir/runtime_checks.rs` 固化并由 final gate 复算。`codegen/emit/ops.rs::emit_pointer_binary` 消费完整 `ExprValue`：equality 只组合 provenance 与 address lane，ordering 先执行 resolved provenance guard 再比较 unsigned address，difference 依次执行 provenance、checked i64 byte distance 与 `stride(T)` lattice checks 后返回 element distance；禁止比较 pointer-view cell 地址、把 bounds lane 错当 identity 或在后端重算 check requirement；

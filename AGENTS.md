...
- pointer equality / ordering 的静态类型兼容性只由 `sema/exprs/operators.rs::binary_result_type` 决定；ordering 的 provenance-check requirement 由 `hir/runtime_checks.rs` 固化并由 final gate 复算。`codegen/emit/ops.rs::emit_pointer_comparison` 消费完整 `ExprValue`：equality 只组合 provenance 与 address lane，ordering 先执行 resolved provenance guard 再比较 unsigned address；禁止比较 pointer-view cell 地址或把 bounds lane 错当 identity；

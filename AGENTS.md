...
- pointer `==` / `!=` 的静态类型兼容性只由 `sema/exprs/operators.rs::binary_result_type` 决定；`codegen/emit/ops.rs::emit_pointer_equality` 消费完整 `ExprValue`，只组合 provenance 与 address lane 的相等结果，禁止比较 pointer-view cell 地址或把 bounds lane 错当 identity；

...
- `hir/runtime_checks.rs` 是“静态已证明 / 必须运行时检查”决策的 canonical owner。当前 array Index 已在 Expr/Place HIR 固化 bounds `RuntimeCheckRequirement`：直接数组字面量的合法常量下标可标记 `Proven`，静态越界直接拒绝，其它数组 Place / 动态下标保持 `Required`；pointer ordering/difference 的 provenance requirement、difference element-lattice requirement、pointer arithmetic overflow/bounds requirement 与 reinterpret alignment requirement 同样固化并由 final gate 独立复算；直接 `refer` arithmetic chain 的常量 offset 可静态证明，非法范围直接拒绝；当前没有 symbolic pointer alignment fact，因此 reinterpret 保持 `Required`，后续 raw-init checks 必须扩展同一 owner，不能让 emitter 从语法形状重建决策；
- raw allocation 使用独立 `RawAllocate { element_ty, count }` / `FreeRawAllocation { pointer }` HIR 节点；全部 HIR 遍历显式访问 operand，避免从 generic-call AST 重建语义。源码 `malloc<T>()` 由专用 generic-intrinsic AST 直接降低为 count=i64(1) 的 nullable allocation-root producer，`free` 只接受 `OwnedTemporary + Available` pointer value；显式 count 形式在其 source integer type 冻结前继续拒绝。不得用普通 BuiltinCall、implicit discard 或 heap-free bit pattern 绕过该边界；
- `reinterpret<T>(source)` 固化为专用 `ReinterpretPointer { target_ty, source, source_origin, alignment_check }` HIR：只接受 non-null borrow-derived pointer view，保持原 provenance/current address，以剩余 bytes 的完整目标 stride 收窄新 lattice，尾部不足一个 stride 的 bytes 不进入结果。它不创建 ownership/root/object/init identity；结果是只能进入 local borrowed slot 的 `BorrowedValue`，由 source origin 传播 reaching loan；
- `hir/ownership_flow.rs` 以显式 CFG/worklist 统一验证 dynamic local 的程序点 capability、local borrow、closure capture loan 与独立 raw-root obligation；branch/loop join 对 may-be-moved / may-be-exposed / may-still-own-raw-root fail-closed，所有可达终点拒绝未 free/transfer 的 local 或 `Owned` parameter pointer root，live root replacement 同样拒绝；reaching-definition + backward liveness 推导每个 loan generation 的 NLL region，并按实际 referent use 固化 ReadLoan/WriteLoan；pointer comparison/difference 对 owning pointer local 使用 non-escaping `InspectPointer` action，对 borrowed pointer 继续要求其来源 loan holder live；borrow-derived pointer arithmetic/reinterpret 共享 resolved `PointerViewSource`，把 source binding 的 reaching loans 或直接 `LoanId` 传播到派生 borrowed slot，禁止 owner write/move 穿过派生 view；未锚定 `OwnedTemporary` pointer operand 直接拒绝，不能用 pointer operation 吞掉 allocation-root obligation；命名 closure binding 与立即调用临时值都是显式 loan holder，嵌套 closure holder dependency 继续保持被捕获 closure 的内层 loans；loan conflict 与 Move replacement 都消费 canonical Place relation，只有 `Disjoint` 授权独立；分析不得把用户 HIR nesting 映射到宿主递归；
...
- `hir/parameter_effects.rs` 在完整 HIR 上以有限格 fixed-point 统一固化 parameter `ReadBorrow / WriteBorrow / Owned` 与 return `Inline / Owned / Borrowed(source)` effects；完整 `Ty::Func`、显式 `Param`、用户方法 target、每个 caller `ArgumentPass` / `CallResult` 与 return `ReturnPass` 必须一致。稳定 dynamic owning Place 只可进入 borrow parameter effect，`Owned` 参数必须接收 `OwnedTemporary + Available`；`BorrowTemporary` 同时携带 canonical `DestroyPlan`，final gate 按实参或 receiver 类型独立复算。allocation-root ptr temporary 不允许用 `BorrowTemporary` 在调用后隐式 free，只能显式 free 或 transfer 给 `Owned` parameter。用户函数/方法调用返回后销毁其它 borrowed temporary referent 并释放 caller cell；`string.len/upper/lower/trim` 与 `array.len/push/pop` 的 owned temporary receiver 同样固化 `BorrowTemporary` pass，并在 builtin 操作完成后直接销毁。稳定 Place 与 borrowed alias receiver 继续消费既有 Place/alias access flow，不制造第二个 call loan。`array.iterator` 的 temporary receiver 必须随 iterator 来源 loan 保持，不能在创建调用后立即销毁。borrowed return source 当前精确到 `Parameter(index) / Self / Global(binding)`，caller 建立 returned loan。call/returned loan 与 local/capture loan 共用 ownership CFG/NLL，final gate 独立复算并拒绝签名、source、source writability、loan kind、temporary destruction 或 pass 漂移；
- `hir/ownership_operations.rs` 在 function effect 写回后统一固化 destination-side write：显式 Binding 初始化使用 `Initialize(InlineCopy|OwnershipTransfer)` 或 `BindBorrowedAlias`，owning Place replacement 使用 `Replace(InlineCopy|OwnershipTransfer)`，直接 borrowed-alias 赋值使用 `RebindBorrowedAlias`。ownership CFG、final gate 与 codegen 必须消费该 operation；effect fixed-point 尚未完成时的 provisional CFG 也只能调用这一 owner 计算草案，不能复制分类矩阵。struct 字段默认值/构造实参、array literal 元素/`push` 实参与 result payload 在直接 RHS 的 `ExprInfo::container_write` 固化同一 `OwningWrite`；final gate 按目标位置独立复算并拒绝缺失、漂移或放错位置的 operation，后端经 `emit_container_value` 消费，源值的 Move/loan 合法性继续由原有表达式 CFG 验证。顶层与局部显式 Binding、`for` element binding、显式 parameter、隐式 self parameter，以及被普通 statement 丢弃的 `OwnedTemporary + Available` expression 同时固化 canonical `DestroyPlan`；match 块的尾 expression 作为 arm 结果 transfer，不执行 statement discard。初始化不是 replacement，不附带销毁旧值的责任。除已由 receiver pass 覆盖的 builtin method receiver 外，其它嵌套临时值与 raw-init 的完整销毁合同仍未完成；
- `hir/pattern_bindings.rs` 在 function effect 写回后为 match arm binding 固化 `InlineCopy / DeepClone(plan) / OwnershipTransfer` 及同类型的 `DestroyPlan`。普通整值 binding 可 transfer fresh owned subject，稳定 Place / BorrowedValue 必须 clone；constructor payload 属于仍 live 的 result storage，只能 inline copy 或 clone，禁止 partial move。ownership CFG 的 provisional 构图只按 canonical binding type 建立 dynamic local，final gate 与 codegen 必须消费并复核冻结 operation/destruction；
...
- 显式 `refer(place)` 固化为专用 `Expr::Refer { loan_id, source: Place, kind }`，并在 `CheckedProgram::address_taken_roots` 精确登记物理 descriptor root；当前开放根植于 current-function owning local 或 owning global 的 whole Local 与直接 Field/Index Place，binding cell 与其 heap struct object / array backing 分属不同 provenance，直接 subview 以 projection address/stride 收窄 view。nested projection 等待逐层 heap descriptor 生命周期，保持 fail-closed。结果是 non-null `ptr<T>` 的 `BorrowedValue + None`，复用 NLL loan、canonical Place overlap 与 stored-borrow gate；parameter/capture source、borrowed pointer return 与显式 BorrowedValue forwarding 继续 fail-closed；
...
`codegen/emit/provenance.rs` 为 `CheckedProgram::address_taken_roots` 中的每个物理 local/global storage root 建立恰好一个 canonical descriptor：whole binding cell 与直接 field/index 所在 heap object/backing 使用不同 descriptor，同一物理 root 的重复 `refer` 才复用 identity；subview address 必须经 `emit/places.rs` 的 resolved Place 地址链取得，不能从 root cell 猜 projection。object descriptor 在 `refer` 时按当前 backing 刷新 base/extent；live loan 禁止 backing relocation。local descriptor 登记到词法 cleanup stack；global descriptor 是 global slab 内的静态记录。当前 pointer view 通过完整四 lane 临时 cell 进入既有 borrowed-alias address carrier，view cell 同样使用词法 cleanup，alias cell 只释放自身。后端不得按 refer 次数重建 descriptor，也不得把 pointer address 当 provenance。
...
`hir/destruction.rs` 是 assignment、顶层与局部显式 Binding、parameter/self、`for` element、Pattern binding 以及 call/for/discard/builtin-receiver temporary 共用的 `DestroyPlan` 唯一 owner，以扁平 child index 固化 struct/array/result 的 owned children、string、closure root/env 与 iterator 自身状态清理；不借用 cloneability 判定，也不把 iterator 的源数组当作 owned child。`ownership_operations` 为 borrowed alias rebind/binding 固化 Inline 空操作，final gate 独立复算并拒绝 recipe 缺失或漂移。`codegen/emit/destruction.rs` 只执行该 recipe：struct 逆声明顺序、array 逆索引、result active payload、children first / root last。

赋值的 `previous_owner` 由 ownership CFG 在 RHS 与目标投影求值后的 replacement 节点固化为 `Unreachable / None / Live / MaybeMoved`；只有完整 worklist 收敛后的输入状态可用于该 fact，循环入口仍 live 而回边已经 moved 的赋值必须为 `MaybeMoved`。final gate 独立复算并拒绝缺失或漂移。codegen 为 dynamic local 使用独立 SSA presence，Move 取值后清零、replacement commit 后置回；replacement 与 scope exit 都禁止检查保留旧 payload 的 cell 位模式。词法 cleanup stack 同时容纳带 BindingId/presence 的 local owner 和无 BindingId 的编译器临时 owner；parameter/self、显式 local、每轮 `for` element 与每个命中的 Pattern binding 在正常 fallthrough、函数退出、`break`、`continue` 上按内层 scope 与逆声明顺序清理；普通 expression statement 若携带 resolved discard plan，则在完整求值后立即销毁其结果 owner；非 iterator builtin method 在完成操作后销毁 resolved `BorrowTemporary` receiver。owning value 执行 resolved recipe，borrowed alias 不销毁 referent，随后均释放 binding cell。`main` 正常返回后，进程入口按源码逆序销毁仍由顶层 owning Binding 持有的值，再释放唯一 global slab；runtime abort 仍直接终止进程。
...
`for` 的 element owning binding 由 ownership CFG 在进入 body 的边上执行 `Declare`，每轮恢复新副本的 capability；不能只在循环前初始化，也不能把 header 的零次迭代退出边视为生成 owner。循环 binding 与普通 local 共用 move/loan/presence 验证，并在每轮 fallthrough、`continue`、`break` 或函数退出时执行 resolved destruction；source_pass 复用 parameter_effects 的 argument-pass owner 并在 final gate 独立复核，loop header 的 UseLoanHolder 保持回边上的 NLL region。若 source 是 `BorrowTemporary`，codegen 在循环专属 cleanup scope 中保持并按其 resolved `DestroyPlan` 销毁 source；从 array 建立的内部 iterator cursor 同样在该 scope deallocate。正常穷尽与 `break` 先清理 cursor 再清理 temporary source，`continue` 保持两者，函数 `return` 通过统一 cleanup stack 清理两者。显式 ArrayIterator receiver_pass 同样复用该 owner；局部 iterator binding 是 loan holder，move 到新 binding 通过 CarryLoans 保留原 source loans，而不是重新借用已经 moved 的 iterator cell。局部 owning iterator replacement 先以独立 RHS holder 求值，通过 Reinitialize 的冲突检查后再用 SetLoans 提交新来源集合；SetLoans 同时是 reaching-definition 的替换边界和 backward liveness 的 kill 边界。临时 holder 分配统一经 ownership CFG 的 temporary_holder，开始求值前清空上一执行轮次的 loans；不得在 for 的内部回边上清空仍需使用的来源。iterator 的容器写入按 stored borrow 禁令拒绝。
...
`borrow_contract::validate_stored_value` 拥有 iterator 的 stored-borrow 禁令：OwnedTemporary cursor 仍携带 non-owning 来源。ownership_operations 的 canonical container write 与非 Local replacement 调用该 owner，global binding 由 containment gate 检查；不能另建容器内持久来源图绕过 plan 第 5 节。
...
- `PtrLayout` 已冻结当前 Windows x64 capability 的 `provenance / address / view_start / view_end` 四个 I64 lane、`0/8/16/24` offset 与 `size=32 / align=8 / stride=32`；编译入口会把它与目标 ISA 的 I64 machine pointer 核对。`Compiler::machine_ptr_ty` 只表示单个原生地址，不能当作 Alias pointer value ABI；pointer type slot / struct layout / Ty→VTy 与 `malloc<T>()`、local/global `refer`、move、用户函数参数/返回、free expression path、equality/ordering/difference、borrow-derived checked arithmetic 与 `reinterpret<T>` typed lattice narrowing 已接通，其它 pointer producer/operator 继续 fail-closed；
...
- pointer equality / ordering / difference / arithmetic 的静态类型兼容性只由 `sema/exprs/operators.rs::binary_result_type` 决定；运行时 check requirement 由 `hir/runtime_checks.rs` 固化并由 final gate 复算。`codegen/emit/ops.rs::emit_pointer_binary` 消费完整 `ExprValue`：equality 只组合 provenance 与 address lane，ordering 先执行 resolved provenance guard 再比较 unsigned address，difference 依次执行 provenance、checked i64 byte distance 与 `stride(T)` lattice checks 后返回 element distance；`codegen/emit/provenance.rs` 发射 checked pointer offset 以及 reinterpret alignment/remaining-byte lattice narrowing，二者都保留 provenance 并物化派生 alias view；禁止比较 pointer-view cell 地址、把 bounds lane 错当 identity 或在后端重算 check requirement；
...
`UserFunctionAbi` 统一决定用户函数机器签名、parameter passing、return passing 与 `[sret?, globals, closure_env, ...]` 的机器参数索引；`VTy::Func` 保留 resolved parameter effects。调用端与函数生成器共同消费这一 owner，borrow parameter 传递 I64 referent address，by-value parameter 消费 `Direct / IndirectByValue`。callee 为两者建立显式 cell：borrowed cell 在退出时只释放自身，Owned cell 按 resolved `DestroyPlan` 清理，已 transfer 的 owner 由 presence 跳过。caller 在调用返回后逆序结束 `BorrowTemporary` 的 resolved owner 生命周期并释放其 cell；`IndirectByValue` argument cell 在 callee 完成按值装载后只 deallocate，不销毁已经 transfer 的值。调用结果与 return 汇合已保留 `ExprValue`，显式 sret 分支写入 caller return area，caller 完整装载 lanes 后立即释放该纯机器 storage。pointer borrowed parameter 的类型/机器签名已可生成；allocation-root pointer owner 已可经 `IndirectByValue` 用户参数和 `ExplicitSRet` return transfer，caller/callee 同样保存全部四个 lane。
...
canonical `StorageDescriptor { base, extent, kind, raw_metadata }` 与当前空态 `RawInitMetadata { regions, count }` 物理布局由 `codegen/layout.rs` 单一拥有。`rt.raw.alloc/free` 已为 raw allocation 建立 fallible allocation、稳定 descriptor identity、失败回滚与空 metadata 释放；address-taken local/global 的 binding cell 与直接 field/index 所在 heap object/backing 各自建立物理 root descriptor，base/extent 指向对应实际 storage，raw metadata 为 null。源码级 `malloc<T>()` / `free`、whole-local 及直接 field/index `refer` 已开放；nested heap projection、`malloc<T>(count)`、initialized-region entry ABI 与 reverse-init destruction 尚未开放，region count 非零时当前 raw free shim 必须 trap，不能在缺少 destruction descriptor 时静默释放。

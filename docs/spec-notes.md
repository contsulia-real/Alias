...
每个用户调用在 HIR 中固化 caller-side argument pass：inline、borrow stable Place、borrow temporary 或 ownership transfer。多个 call loan 的 NLL region 覆盖从实参求值到该次调用消费 holder 的区间，并与 local/capture/returned loan 使用同一 canonical Place overlap 规则。callee parameter/self cell 在所有函数出口释放；borrow effect 不销毁 referent，Owned effect 按 resolved `DestroyPlan` 清理仍 present 的 owner。显式 BorrowedValue/reborrow forwarding 仍未落地，因此不会借 parameter effect 名义偷渡逃逸引用。

...
- `for` element 与 Pattern binding 在各自词法出口执行 resolved destruction；parameter、表达式临时值、global 所持有的 struct/result/array/iterator 等仍 live 对象尚未在对应生命周期出口回收；

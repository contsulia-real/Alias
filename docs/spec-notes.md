...
- parameter/self、`for` element 与 Pattern binding 在各自词法出口执行 resolved destruction；用户函数/方法的 borrowed temporary receiver/argument 在调用返回后执行 resolved destruction 并释放 caller cell，纯机器 `IndirectByValue`/sret storage 在值完成转移或装载后释放；直接作为 `for` source 的 temporary 在正常穷尽、`break` 与函数 `return` 上清理，array source 对应的内部 iterator cursor 同步释放，`continue` 保持两者；普通 statement 丢弃的 owned expression result 在完整求值后销毁；`main` 正常返回后，顶层 owning Binding 按源码逆序销毁所持值并释放 global slab；其它嵌套表达式临时值尚未在对应生命周期出口完整回收；
